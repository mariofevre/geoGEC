...
$query="
	...
	FROM 
		geogec.ref_proc_procesos as pr
		
	LEFT JOIN 
		geogec.ref_proc_componentes as pr_co
		ON
...
while ($fila=pg_fetch_assoc($Consulta)){
	...
	$ruta=$GeoGecPath.'/app_proc/procesos';
...
$query="
	...
	FROM
		geogec.ref_proc_componentes_precedentes as pr
	LEFT JOIN
		geogec.ref_proc_componentes as copre
		ON copre.id = pr.id_p_ref_proc_componente_precedente 
	LEFT JOIN 
		geogec.ref_proc_componentes as copos
